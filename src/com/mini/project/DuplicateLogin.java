...
public class DuplicateLogin {
	...
	public void getLoginVerifiaction() throws SQLException {
		...
		try {
			...
			PreparedStatement preparedStatement = connection2.prepareStatement("select username from quiz where username=?");
			PreparedStatement preparedStatement2 = connection2.prepareStatement("select password from quiz where password=?");
			...
			if(username.equals(uName) && password.equals(pass)) {
				DataBaseQuestions dataBaseQuestions = new DataBaseQuestions();
				dataBaseQuestions.getDataBaseQuestions();
